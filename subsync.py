...
def get_file_paths(filedir):
    print(f"Provided directory: {filedir}")

    files = []
    for filename in os.listdir(filedir):
        file_path = os.path.abspath(os.path.join(filedir, filename))
...
def fix_styling(filedir):
    files = [os.path.abspath(os.path.join(filedir, filename)) for filename in os.listdir(filedir)]

    extracted_subs = [f for f in files if '.EXTRACTED.' in f]
...
def retime(mkv, srt, conf, filedir):
    print(f'mkv {mkv}')
    print(f'filedir {filedir}')
    print(srt)
    print(conf)
    retimed = mkv.replace(".mkv", ".ja.RETIMED.{}".format(conf['ext']))
    extracted = [f for f in get_file_paths(filedir) if mkv.replace(".mkv", ".EXTRACTED.") in f][0]
...
if __name__ == '__main__':
    ...
    if "2" in retime_choice:
        ...
        fix_styling(filedir)
        print(filedir)
        pool.starmap(retime, zip(mkvs, srts, [CONF]*len(mkvs), [filedir]*len(mkvs)))
    print("\nSuccess!\n")
    time.sleep(3)
